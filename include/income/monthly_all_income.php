	...
	<div class="box box-default">
		<div class="box-header with-border">
			<h3 class="box-title"> Monthly Income Report</h3>
		...
		<div class="box-body">
			...
			<form method="POST" action="home.php?item=34">
				...
				<select class="selectpicker" data-show-subtext="true" data-live-search="true" data-size="10" name="txtMonth">
					<option>--Month--</option>
					<?php
					  $month_table = $db->query("select id,name from bf_month");
					  while(list($id,$name)=$month_table->fetch_row()){
		 ...
		 if(isset($_POST["btnSearch"])) : ?>
		 	...
		 	$yearlyincome_table = $db->query("select s.id,s.name,i.id,i.amount,i.income_date from bf_source as s,bf_income as i where s.id=i.source_id and year(i.income_date) like '%".$year."%' and month(i.income_date) like '%".$month."%'");
		...
		<?php
			...
			$year  = $_POST["txtYear"];
		 	$month = $_POST["txtMonth"];
				...
				$yearlyincome_table = $db->query("select s.id,s.name,i.id,sum(i.amount),i.income_date from bf_source as s,bf_income as i where s.id=i.source_id and year(i.income_date) like '%".$year."%' and month(i.income_date) like '%".$month."%' ");
				while(list($sid,$sname,$iid,$amount,$date)=$yearlyincome_table ->fetch_row()) : ?>
