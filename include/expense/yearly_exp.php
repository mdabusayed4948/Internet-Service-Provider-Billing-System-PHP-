	...
	<div class="box box-default">
		<div class="box-header with-border">
			<h3 class="box-title"> Yearly Expense Report</h3>
		...
		<div class="box-body">
			<div class="row">
				<div class="col-md-12">
			<form action="home.php?item=50" method="post">
	...
	<button class="btn btn-default" onclick="myFunction('location.reload()')"> <i class="glyphicon glyphicon-refresh"></i> </button>
		...
		<?php
		 if(isset($_POST["btnSearch"])) : ?>
		 	...
		 	$yearlyexp_table = $db->query("select s.id,s.name,ex.id,ex.amount,ex.exp_date from bf_source as s,bf_expense as ex where s.id=ex.source_id and year(ex.exp_date) like '%".$yearly_exp."%' ");
		...
		<?php
			...
			$yearly_income = $_POST["txtSearch"];
				...
				$yearlyincome_table = $db->query("select s.id,s.name,ex.id,sum(ex.amount),ex.exp_date from bf_source as s,bf_expense as ex where s.id=ex.source_id and year(ex.exp_date) like '%".$yearly_exp."%' ");
				...
				<tfoot style="background-color: gray; color: #FCFCFC">
					<tr>
						<th colspan="2" style="font-size: 20px">Total Yearly Expense :
