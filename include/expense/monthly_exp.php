	...
	<div class="box box-default">
		<div class="box-header with-border">
			<h3 class="box-title"> Monthly Expense Report</h3>
		...
		<div class="box-body">
			...
			<form method="POST" action="home.php?item=49">
		 ...
		 if(isset($_POST["btnSearch"])) : ?>
		 	...
		 	$yearlyincome_table = $db->query("select s.id,s.name,ex.id,ex.amount,ex.exp_date from bf_source as s, bf_expense as ex where s.id=ex.source_id and year(ex.exp_date) like '%".$year."%' and month(ex.exp_date) like '%".$month."%'");
		...
		<?php
			...
			$year  = $_POST["txtYear"];
		 	$month = $_POST["txtMonth"];
				...
				$yearlyincome_table = $db->query("select s.id,s.name,ex.id,sum(ex.amount),ex.exp_date from bf_source as s, bf_expense as ex where s.id=ex.source_id and year(ex.exp_date) like '%".$year."%' and month(ex.exp_date) like '%".$month."%' ");
				while(list($sid,$sname,$iid,$amount,$date)=$yearlyincome_table ->fetch_row()) : ?>
